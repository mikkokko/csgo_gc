...
std::string ItemSchema::GetRarityColorCode(uint32_t rarity) const
{
    // CS:GO rarity color codes (using hex RGB format as text color codes)
    // These colors match the in-game item rarity colors
    switch (rarity)
    {
        case RarityCommon:      return "\x0B";        // Light Blue
        case RarityUncommon:    return "\x0C";        // Light Blue (slightly different)
        case RarityRare:        return "\x09";        // Purple/Magenta
        case RarityMythical:    return "\x0D";        // Pink/Light Red
        case RarityLegendary:   return "\x02";        // Red
        case RarityAncient:     return "\x02";        // Dark Red
        case RarityImmortal:    return "\x10";        // Gold/Yellow
        default:                return "\x01";        // Default White
    }
}

std::string ItemSchema::GetFormattedItemName(const CSOEconItem &item) const
{
    ...
    // Add rarity color code
    uint32_t rarity = item.rarity();
    result += GetRarityColorCode(rarity);
    
    // Add item name
    result += itemInfo->m_name;
