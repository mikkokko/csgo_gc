...
public:
    ...
    // Get rarity color code for chat messages (using CS:GO item rarity colors)
    std::string GetRarityColorCode(uint32_t rarity) const;
    // Get formatted item name with rarity color
    std::string GetFormattedItemName(const CSOEconItem &item) const;
