...
void ClientGC::Craft(GCMessageRead &messageRead)
{
    ...
    if (m_inventory.TradeUp(inputItemIds, destroyItems, newItem, notification, &craftedItem))
    {
        ...
        // Send chat notification to all players on the server
        if (craftedItem)
        {
            std::string formattedItemName = m_inventory.GetItemSchema().GetFormattedItemName(*craftedItem);
            
            // Format: [Player Name] has fulfilled a contract and received: [Item Name]
            // Since we don't have direct access to player name in the GC, we'll use the account ID
            // The game server can replace this with the actual player name if needed
            uint32_t accountId = m_steamId & 0xffffffff;
            std::string chatMessage = "Account " + std::to_string(accountId) + " has fulfilled a contract and received: " + formattedItemName;
            
            // Create a text message for broadcasting
            CMsgGCCStrike15_v2_GC2ClientTextMsg textMsg;
            textMsg.set_id(0);
            textMsg.set_type(0);  // type 0 for generic text message
            ...
            Platform::Print("Trade-up chat notification sent\n");
        }
